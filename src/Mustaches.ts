...
class Mustaches {
  ...
  async interpolate({
    ...
    updates.sort((a, b) => {
      const hasPlaceholdersInFilterParams = ({
        replaceAllText: { containsText: { text = '' } = {} } = {}
      }) =>
        text.split(/\| (.+)?/, 2).length > 1 &&
        text
          .split(/\| (.+)?/, 2)
          .slice(1)
          .some(i => /\[\[([^}]*)]]/.test(i))
      if (a.insertInlineImage && b.insertInlineImage) {
        ...
        else return b.insertInlineImage.location.index - a.insertInlineImage.location.index
      } else if (hasPlaceholdersInFilterParams(a) && hasPlaceholdersInFilterParams(b)) {
        return 0
      } else if (a.insertInlineImage) {
        return -1
      } else if (hasPlaceholdersInFilterParams(a) && !b.insertInlineImage) {
        return -1
      } else {
        return 1
      }
