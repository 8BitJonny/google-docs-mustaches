...
class Mustaches {
    ...
    async interpolate({ source, destination, name, data, formatters = {}, strict }) {
        ...
        updates.sort((a, b) => {
            const hasPlaceholdersInFilterParams = ({ replaceAllText: { containsText: { text = '' } = {} } = {} }) => text.split(/\| (.+)?/, 2).length > 1 &&
                text
                    .split(/\| (.+)?/, 2)
                    .slice(1)
                    .some(i => /\[\[([^}]*)]]/.test(i));
            if (a.insertInlineImage && b.insertInlineImage) {
                if (!b.insertInlineImage.location || !a.insertInlineImage.location)
                    return 0;
                else
